    ...
    public void explosionTick(IExplosiveEntity entity) {
        if(entity instanceof LTNTEntity) {
            ...
            if(entity.getTNTFuse() < 60 && entity.getTNTFuse() % 3 == 0) {
                ...
                compound.putFloat("spiral_power", MathHelper.clamp(compound.getFloat("spiral_power") + 0.06f, 0.2f, Float.MAX_VALUE));
                ent.readNbt(compound);
                ...
                spiral_tnt.setVelocity((Entity) entity, (float) ent.getRotationVecClient().x, (float) ent.getRotationVecClient().y+(getDefaultFuse(entity)-entity.getTNTFuse())*10, (float) ent.getRotationVecClient().z, ent.writeNbt(compound).getFloat("spiral_power"), 0);
